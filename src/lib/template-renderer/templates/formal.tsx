...
function buildFormalSectionHtml(section: CanonicalResume['sections'][number], lang: string): string {
  ...
  if (section.type === 'github') {
    const items = (content as unknown as GitHubContent).items || [];
    return `<div class="space-y-3">${items.map((item) => `<div>
      <div class="flex items-baseline justify-between"><span class="text-sm font-bold" style="color:${DARK_GREEN}">${esc(item.name)}${item.repoUrl ? ` <a href="${esc(item.repoUrl)}" target="_blank" rel="noopener noreferrer" class="ml-1 text-xs font-normal" style="color:#3b82f6">${esc(item.repoUrl)}</a>` : ''}</span><span class="shrink-0 text-xs italic text-zinc-400">★ ${item.stars?.toLocaleString() ?? 0}</span></div>
